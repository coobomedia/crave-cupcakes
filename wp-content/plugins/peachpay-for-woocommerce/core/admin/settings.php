 ...
 */
function peachpay_display_premium_banner() {
	?>
		<div class='notice notice-warning' style='display: flex; justify-content: space-between; align-items: center;'>
			<div>
				<p>
					<?php echo esc_html__( 'This is a Premium feature.', 'peachpay-for-woocommerce' ); ?>
					<?php echo esc_html__( 'Upgrade to unlock it!', 'peachpay-for-woocommerce' ); ?>
				</p>
			</div>
			<button type="button" class="pp-button-continue-premium button pp-button-primary"><?php echo esc_html__( 'Upgrade', 'peachpay-for-woocommerce' ); ?></button>
...
function peachpay_options_page_html() {
	...
	update_option( 'peachpay_premium_capability', peachpay_plugin_get_capability( 'woocommerce_premium', $plugin_capabilities ) );
	...
	$premium_locked_flag = ! peachpay_plugin_has_capability( 'woocommerce_premium', $plugin_capabilities ) && in_array( $tab, array( 'currency', 'field', 'related_products', 'express_checkout' ), true );

	require PeachPay::get_plugin_path() . '/core/admin/views/html-primary-navigation.php';
	if ( $premium_locked_flag ) {
		peachpay_display_premium_banner();
	}
	if ( 'home' === $tab ) {
