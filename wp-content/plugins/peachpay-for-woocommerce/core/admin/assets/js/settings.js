...
document.addEventListener('DOMContentLoaded', initHomepageEvents);
...
function setupLeaveListener() {
	...
	const exceptionElements = Array.from(listenForChangeForm.querySelectorAll('input[name=select_field], .select-all, #peachpay-register-landing'));
...
function initPreviews() {
	...
	if (urlParams.get('tab') === 'express_checkout' && urlParams.get('section') === 'button') {
		initShopPagePreview();
		initPreviewButton();
		initCheckoutButtonEventListeners();
		setEffect();
	}
	if (urlParams.get('tab') === 'express_checkout' && urlParams.get('section') === 'branding') {
...
/**
 * Initializes homepage UI states and event listeners.
 */
function initHomepageEvents() {
	document.querySelectorAll('.pp-homepage-section-card-sub-logos-toggle').forEach(($button) => {
		$button.addEventListener('click', () => {
			const $subLogosContainer = $button.closest('.pp-homepage-section-card-sub-logos');
			if ($subLogosContainer.hasAttribute('expanded')) {
				$subLogosContainer.removeAttribute('expanded');
				$subLogosContainer.querySelector('.pp-homepage-section-card-sub-logos-toggle').innerHTML = 'See more';

			} else {
				$subLogosContainer.setAttribute('expanded', null);
				$subLogosContainer.querySelector('.pp-homepage-section-card-sub-logos-toggle').innerHTML = 'See less';
			}
		});
	});
}