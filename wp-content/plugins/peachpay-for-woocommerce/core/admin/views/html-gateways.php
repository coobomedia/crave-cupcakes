...
foreach ( $gateway_list as $gateway ) : ?>
	<div class="gateway">
		<div class="flex-col gap-12 w-100">
			<div class="list-item-heading gap-12">
				...
				<?php if ( 'yes' === $gateway->enabled && ! $gateway->needs_setup() ) { ?>
					<div class="flex-row gap-8">
						<div class="general active">Active</div>
					</div>
				<?php } else { ?>
					<div class="general inactive">Inactive</div>
				<?php } ?>
			</div>
			<?php if ( 'yes' === $gateway->enabled && ! $gateway->needs_setup() ) { ?>
		...
		<div class="buttons-container flex-col <?php echo esc_attr( $gateway->needs_setup() ? 'needs-setup' : ( ( 'yes' === $gateway->enabled ) ? 'enabled' : 'disabled' ) ); ?>">
			...
			<div class="activate-button" data-heap="<?php echo 'activate_' . esc_html( $gateway->id ); ?>" onclick="stopPopupOnSubmit()" data-id="<?php echo esc_html( $gateway->id ); ?>">
				<?php
				esc_html_e( 'Activate', 'peachpay-for-woocommerce' );
				?>
				<span class="spinner"></span>
			</div>
			<div class="deactivate-button" data-heap="<?php echo 'deactivate_' . esc_html( $gateway->id ); ?>" onclick="stopPopupOnSubmit()" data-id="<?php echo esc_html( $gateway->id ); ?>">
				<?php
