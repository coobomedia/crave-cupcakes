...
<!-- bakeshop table  -->
			<table class="crave-table" style="margin-top:10px">
				<tr>
					<th class="row-title"><b><?php esc_attr_e('Cake / Bakeshop Items', 'WpAdminStyle'); ?></b></th>
			...
			foreach ($cake_bakeshop_item as $bakeshop_item) {	
				...
				foreach ($time_array as $key2 => $new_time) { 
				 ...
				 $product_id =  $new_product[$key2]['product_id'];
						$metaID = get_post_meta( $product_id, 'cakebakeshop_items', true );
						 
					if($bakeShop_term_id == $metaID)
				{ 
					$totalQty = $totalQty+$new_product[$key2]['quantity'];
